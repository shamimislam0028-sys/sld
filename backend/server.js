...
const { extractMcqsWithGemini, lineDataToText, hasGeminiKey } = require('../api/gemini-extract');
...
function ommlToLatex(xml) {
  ...
  for (let iter = 0; iter < 40 && prev !== s; iter++) {
    ...
    s = s.replace(/<m:m(?:\s[^>]*)?>([\s\S]*?)<\/m:m>/g, (_, body) => {
      const rows = [];
      const mrRx = /<m:mr(?:\s[^>]*)?>([\s\S]*?)<\/m:mr>/g;
      let rm;
      while ((rm = mrRx.exec(body)) !== null) {
        const cells = [];
        const eRx = /<m:e(?:\s[^>]*)?>([\s\S]*?)<\/m:e>/g;
        let em;
        while ((em = eRx.exec(rm[1])) !== null) cells.push(clean(em[1]));
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // Primary: Gemini. Fallback: regex parser (missing key / exhausted quota).
      let mcqs = [];
      if (hasGeminiKey()) {
        try {
          mcqs = await extractMcqsWithGemini(lineDataToText(lineData));
        } catch (aiErr) {
          console.error('Gemini failed, using regex fallback:', aiErr.message);
      ...
      console.log('DEBUG: Parsed MCQs:', mcqs.length);
