...
function getScaleFactor(len, isQuestion) {
  // Run-level shrinking is disabled: auto-fit is now handled entirely by an
  // explicit normAutofit fontScale computed from the box geometry (see
  // computeFontScale). Keeping the run at the template size lets that
  // computation read the true base font size. Returning 1.0 keeps the run
  // size unchanged while leaving the call sites intact.
  return 1.0;
}
...
/**
 * Effective text length: math blocks ($$...$$) render compact regardless of how
 * long their LaTeX is, so count each as a few chars instead of its full length.
 */
function effectiveLen(text) {
  const mathRx = /\$\$[\s\S]*?\$\$|\$[^$\n]*?\$/g;
  const mathCount = (text.match(mathRx) || []).length;
  const nonMath = text.replace(mathRx, '');
  return nonMath.length + mathCount * 4;
}

/**
 * Calibrated shrink factor for the FIXED question / option boxes in the master
 * template (question box ≈ 40pt base, option box ≈ 36pt). Length-based because
 * the boxes are grouped (their raw <a:ext> is in child coordinates, so real
 * geometry can't be read reliably). Tuned so long content actually fits.
 */
function autofitScale(textLen, isQuestion) {
  if (isQuestion) {
    if (textLen <= 60) return 1.0;
    if (textLen <= 100) return 0.85;
    if (textLen <= 150) return 0.72;
    if (textLen <= 220) return 0.60;
    if (textLen <= 320) return 0.50;
    if (textLen <= 450) return 0.42;
    return 0.35;
  }
  if (textLen <= 20) return 1.0;
  if (textLen <= 40) return 0.82;
  if (textLen <= 65) return 0.68;
  if (textLen <= 95) return 0.56;
  if (textLen <= 140) return 0.46;
  return 0.40;
}
...
function setAutofitTag(txBody, tag) {
  ...
  if (/<a:bodyPr\b[^>]*\/>/.test(txBody)) return txBody.replace(/<a:bodyPr\b([^>]*)\/>/, `<a:bodyPr$1>${tag}</a:bodyPr>`);
  if (/<a:bodyPr\b[^>]*>[\s\S]*?<\/a:bodyPr>/.test(txBody)) return txBody.replace(/(<a:bodyPr\b[^>]*>)/, `$1${tag}`);
  return txBody;
}
...
function ensureNormAutofit(txBody, newText, isQuestion) {
  ...
  // shrink every run font size (a:rPr / m:rPr / endParaRPr all carry sz)
  const shrunk = txBody.replace(/sz="(\d+)"/g, (m, n) => `sz="${Math.max(400, Math.round(+n * scale))}"`);
  return setAutofitTag(shrunk, '<a:noAutofit/>');
}

/**
 * Remove embedded font data + references so the output stays small enough for
 * Vercel's 4.5 MB serverless response limit. Cleans the font files, the
 * <p:embeddedFontLst> in presentation.xml, and its relationships.
 */
async function stripEmbeddedFonts(zip) {
  const fontFiles = [];
  zip.forEach((p) => { if (/^ppt\/fonts\//i.test(p)) fontFiles.push(p); });
  fontFiles.forEach((p) => zip.remove(p));

  const presF = zip.file('ppt/presentation.xml');
  if (presF) {
    let xml = await presF.async('string');
    xml = xml
      .replace(/<p:embeddedFontLst>[\s\S]*?<\/p:embeddedFontLst>/g, '')
      .replace(/\s+embedTrueTypeFonts="[^"]*"/g, '');
    zip.file('ppt/presentation.xml', xml);
  }

  const relsF = zip.file('ppt/_rels/presentation.xml.rels');
  if (relsF) {
    let rels = await relsF.async('string');
    rels = rels.replace(/<Relationship\b[^>]*Type="[^"]*\/font"[^>]*\/>/g, '');
...
function setShapeText(xml, shapeId, newText) {
  ...
  if (isQuestion || isOption) {
    txBody = ensureNormAutofit(txBody, newText, isQuestion);
  }
